  ...
  return (
    <main className="portal-page">
      ...
      <div className="portal-shell">
        <header className="site-header"><button className="wordmark" onClick={() => setSelected(null)}>the<span>morning</span>post</button><a className="prodia-credit" href="https://app.prodia.com/models/minimax/h3/fast" target="_blank" rel="noopener noreferrer" aria-label="Powered by Prodia — opens in a new tab"><img src="/assets/powered-by-prodia-badge.png" alt="Powered by Prodia" width="2172" height="724" /></a></header>
        ...
        {selected ? <Article story={selected} playing={playing} setPlaying={setPlaying} onBack={() => setSelected(null)} /> : (
          <div className="portal-grid">
            <aside className="left-col"><ModuleTitle>USEFUL LINKS</ModuleTitle><div className="fun-box"><div className="fun-icon euro">€</div><a href="/funny/euro-2028-tickets" target="_blank" rel="noopener noreferrer">Buying or selling Euro 2028 tickets?</a><p><a href="/funny/euro-2028-tickets" target="_blank" rel="noopener noreferrer">Click here first!</a></p></div><div className="fun-box"><div className="fun-icon arrow-box">➜<small>TRY THIS!</small></div><a href="/funny/online-auction-payments" target="_blank" rel="noopener noreferrer">Find out about online auction payments</a><p>with first-e the internet bank</p></div><ModuleTitle>MUSIC</ModuleTitle><div className="music-box"><b>NOW PLAYING:</b><a> Cameron’s World Mix</a><audio controls src="/assets/cameronsworld.mp3" /></div><div className="side-ad">ADVERTISEMENT<br /><strong>WIN A HOLIDAY<br />TO SLOUGH!</strong><button>CLICK HERE</button></div>
              <a className="nhs-careers-banner" href="https://www.jobs.nhs.uk/" target="_blank" rel="noopener noreferrer" aria-label="Start your career in the NHS — visit NHS Jobs (opens in a new tab)" title="Independent link to NHS Jobs">
                <img src="/assets/nhs-careers-retro.png" alt="Start Your Career in the NHS. Learn more." width={1024} height={1024} loading="lazy" />
              </a>
            </aside>
            <section className="centre-col"><div className="live-tv"><div className="tv-label">LIVE NEWS <span>● ON AIR</span></div><InfiniteTV priorityClip={readyClip} onPlaying={clip => { if (clip.id === readyClip?.id) { setGenerationStatus("YOUR REPORT IS NOW ON AIR"); setReadyClip(undefined); } else { setGenerationStatus(status => status === "YOUR REPORT IS NOW ON AIR" ? "REPORT BROADCAST — LIVE TV CONTINUES" : status); } }} /></div>
              <section className="website-invitation" aria-labelledby="website-invitation-title">
                ...
                <form className="website-submit" onSubmit={sendWebsite}>
                  ...
                  <button type="submit" disabled={sent}>{sent ? "ADDRESS SENT!" : "SEND WEBSITE"}<span aria-hidden="true"> »</span></button>
                </form>
